...
If "blueprint": Extract every distinct shape and dimension label from the drawing. Do not assume the drawing represents a building, rooms, or walls — it could equally be a cabinet, a furniture part, a mechanical component, or any other object. If the sketch is drawn on graph/grid paper, ignore the background grid entirely — it is not part of the drawing and must never be extracted as shapes or lines. Return coordinates normalized to a 0-1000 x 0-1000 grid (0,0 is top-left). Represent rectangular parts or spaces as "rect" shapes. Represent any other straight edges or segments — including non-rectangular outlines, individual segments, or standalone lines — as "line" shapes when they don't form a complete rect. Edges that are clearly intended to be horizontal or vertical (the vast majority in this kind of sketch) must be reported as exactly horizontal or vertical — snap out any slight skew from hand-drawing rather than reproducing it; only report a non-90-degree angle when the sketch unambiguously shows a deliberate diagonal. Capture every dimension label (e.g. "12'-6\\"", "300mm", "24") as a separate label with its position and text exactly as written. Only include a shape's "label" or a dimension label if that text is actually written on the sketch — never invent, infer, or add names, titles, or dimensions that are not present in the image.
